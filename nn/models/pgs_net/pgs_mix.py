...
class PGS_MIX(BaseModel):
    # Per-group-softmax net
    def build(self):
	print "build PGS_MIX_NET" 
	params = self.params
	...
	Xmask2 = tf.placeholder('float32', shape=[N, A], name='xmask2')
	...
	with tf.name_scope("Fully-connected"):
	    ...
	    with tf.variable_scope("Layer1"):
	       	ytmp = fully_connected(Xtmp, num_neurons=100, name='W1', is_training = is_training, use_batch_norm=True, use_drop_out=False, keep_prob = 0.7, activation = 'relu', default_batch = params.default_batch)	
	    with tf.variable_scope("Layer2"):
	    	ytmp = fully_connected(ytmp, num_neurons=50, name='W2', is_training = is_training, use_batch_norm=True, use_drop_out=False, keep_prob = 1.0, activation = 'sigmoid', default_batch = params.default_batch)
	    with tf.variable_scope("Layer4"):
                ytmp = fully_connected(ytmp, num_neurons=25, name='W4', is_training = is_training, use_batch_norm=True, use_drop_out=False, keep_prob = 1.0, activation = 'sigmoid', default_batch = params.default_batch)
	...
	with tf.name_scope('LogLoss'):
	    ypx = tf.reshape(ytmp,[N, A]) * Xmask2
            ypx = tf.maximum(ypx,1e-5)
            ypx = tf.minimum(ypx,1-1e-5)
            lossx = -(y*tf.log(ypx)+(1-y)*tf.log(1-ypx))
            lossx = tf.reduce_mean(lossx)	
	
	with tf.name_scope('Loss'):
            # Cross-Entropy loss
	    cross_entropy = tf.nn.softmax_cross_entropy_with_logits(yp, y)
            loss = tf.reduce_mean(cross_entropy)
            total_loss = loss + params.logweight*lossx + params.weight_decay * tf.add_n(tf.get_collection('l2'))
	...
	for i in range(N):
	    ...
	    Xmask2[i,0:end-start] = 0
	    #print rtmp, X.shape, Y.shape, y.shape 
	    #X[i,rtmp,:] = x[rtmp,:]
	    #Y[i,rtmp,:] = y[np.array(rtmp)]
	    #Xmask[i,rtmp,:] = 0

	return X, Y, Xmask, Xmask2

    def get_feed_dict(self, batch, is_train):
        X, Y, Xmask, Xmask2 = self.preprocess_batch(batch)
        return {
            self.x: X,
            self.xmask: Xmask,
	    self.xmask2: Xmask2,
