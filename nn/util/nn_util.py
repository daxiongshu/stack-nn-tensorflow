...
def batch_norm(x, is_training, default = False):
    """ Batch normalization.
    :param x: Tensor
...
def fully_connected(input, num_neurons, name, use_batch_norm=False, use_drop_out=False, keep_prob = 1.0, is_training = True , activation = 'relu', default_batch = 0):
    ...
    if use_drop_out:
	l = dropout(l, keep_prob, is_training)
    if activation == 'relu':
	l = tf.nn.relu(l)
